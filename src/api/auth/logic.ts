...
const createUserTokenPayload = (username: string, role: Role = USER_ROLE): TokenPayload =>
  ({ username, role });
...
export const createAuthLogic = (
  ...
  signUp: async (username: string, password: string) => {
    ...
    const token = sign(createUserTokenPayload(username), jwtSecret, { expiresIn: '2d' });
    return token;
  },
  ...
  login: async (username: string, password: string) => {
    ...
    const token = sign(createUserTokenPayload(user.name, user.role), jwtSecret, { expiresIn: '2d' });
    return token;
  },
