...
export const createAuthRouter = (authConfig: AuthConfig, mailer: Mailer, userDal: UserDal) => {
   ...
   * /auth/native/poll:
   *   get:
   *     summary: Poll for iOS native OAuth result
   *     description: >
   *       Used by iOS native clients after opening Google or Apple OAuth in SFSafariViewController.
   *       Because SFSafariViewController cannot redirect to custom URL schemes, iOS passes a nonce
   *       when initiating OAuth and polls this endpoint until the backend resolves the result.
   *       Returns pending while the user has not yet completed sign-in. On completion, returns
   *       a one-time code that can be exchanged via POST /auth/google/native/exchange.
   *     tags:
   *       - Auth
   *     parameters:
   *       - in: query
   ...
   *         schema:
   *           type: string
   *         description: UUID generated by the iOS client before opening the OAuth browser
   *     responses:
   *       200:
   ...
   *                 status:
   *                   type: string
   *                   enum: [pending, success, pending_approval, error]
   *                 code:
   *                   type: string
   *                   description: One-time code present when status is success; exchange via /auth/google/native/exchange
   *                 provider:
   *                   type: string
   *                   enum: [google, apple]
   ...
   *                   type: string
   *                   enum: [conflict]
   *                   description: Present when status is error; conflict means the account uses password login
   *       400:
   *         description: Missing or invalid nonce
   *       404:
   *         description: Nonce not found or expired (TTL is 5 minutes)
   */
  router.get('/native/poll', authHandlers.nativePoll);
