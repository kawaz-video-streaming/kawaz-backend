...
| `POST` | `/admin/pending/:username/approve/:role` | Yes (admin only) | Approve a pending user with an assigned role; sends approval email |
...
interface Profile {
  ...
  watchlist: string[];                  // mediaId references
}
...
interface User {
  ...
  role: string;                         // defaults to "user"; set at admin approval time
