...
| `GET` | `/auth/google/device/poll` | No | Poll for device authorization status using `?device_code=...`; returns `{ status: "pending" \| "slow_down" \| "authorized", username?, role? }`. Sets `kawaz-token` cookie when authorized |
| `POST` | `/auth/google/native/exchange` | No | Exchange the short-lived one-time code from the native callback URL (`{ code }`) for the `kawaz-token` cookie; code expires in 60 seconds and is invalidated on first use |
...
- **BucketsConfig**: All storage bucket names and key prefixes are consolidated into a single `BucketsConfig` object (see `src/utils/types.ts`) passed down to media, mediaCollection, and upload consumer — no per-feature config interfaces for storage. Special users share the same storage buckets as regular users; isolation is enforced entirely at the DB layer.
